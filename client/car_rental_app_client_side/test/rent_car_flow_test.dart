...
void main() {
  group('Rent Out Your Car Flow Tests', () {
    testWidgets('Step 1: Car Specification form validation and navigation', (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1080, 2400);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      await tester.pumpWidget(
        const MaterialApp(
          home: CarSpecificationScreen(),
        ),
      );
      ...
      await tester.enterText(find.widgetWithText(TextFormField, 'Brand'), 'Toyota');
      await tester.enterText(find.widgetWithText(TextFormField, 'Model'), 'Camry');
      await tester.enterText(find.widgetWithText(TextFormField, 'Variant'), 'SE 2.5L');
      await tester.enterText(find.widgetWithText(TextFormField, 'Manufacturing Year'), '2023');
      await tester.enterText(find.widgetWithText(TextFormField, 'Registration Number'), 'KA-05-MN-9988');

      // Select Dropdowns
      await tester.tap(find.widgetWithText(DropdownButtonFormField<String>, 'Fuel Type'));
      await tester.pumpAndSettle();
      await tester.tap(find.text('Petrol').last);
      await tester.pumpAndSettle();

      await tester.tap(find.widgetWithText(DropdownButtonFormField<String>, 'Transmission'));
      await tester.pumpAndSettle();
      await tester.tap(find.text('Automatic').last);
      await tester.pumpAndSettle();

      // Fill numerical fields
      await tester.enterText(find.widgetWithText(TextFormField, 'Mileage'), '16.5');
      await tester.enterText(find.widgetWithText(TextFormField, 'Seating Capacity'), '5');
      await tester.enterText(find.widgetWithText(TextFormField, 'Color'), 'Black');
      await tester.enterText(find.widgetWithText(TextFormField, 'Engine Capacity'), '2500');
      await tester.enterText(find.widgetWithText(TextFormField, 'Odometer Reading'), '15000');
      await tester.enterText(find.widgetWithText(TextFormField, 'Description'), 'Well maintained family car.');
    ...
    test('VehicleModel.fromDraft maps fields correctly for backend submission', () {
      const draft = RentCarDraft(
        ...
        deliveryFee: '400',
        selectedPhotos: ['https://example.com/front.jpg', 'https://example.com/rear.jpg'],
      );
