...
class _CarSpecificationScreenState extends State<CarSpecificationScreen> {
	...
	void initState() {
		...
		_yearController = TextEditingController(text: widget.draft.manufacturingYear);
		_registrationController = TextEditingController(text: widget.draft.registrationNumber);
		_mileageController = TextEditingController(text: widget.draft.mileage);
		_seatingController = TextEditingController(text: widget.draft.seatingCapacity);
		_colorController = TextEditingController(text: widget.draft.color);
		_engineController = TextEditingController(text: widget.draft.engineCapacity);
		_odometerController = TextEditingController(text: widget.draft.odometerReading);
		_descriptionController = TextEditingController(text: widget.draft.vehicleDescription);
		_fuelType = widget.draft.fuelType.isEmpty ? null : widget.draft.fuelType;
		_transmission = widget.draft.transmission.isEmpty ? null : widget.draft.transmission;
	}
	...
	String? _yearValidator(String? value) {
		...
		if (parsed == null || text.length != 4 || parsed < 1980 || parsed > DateTime.now().year + 1) {
	...
	Widget build(BuildContext context) {
		return Form(
			...
			child: RentCarScreenScaffold(
				...
				subtitle: 'Capture the vehicle identity and technical details with clear, structured inputs.',
							...
							child: Column(
								children: [
									RentCarDropdownField<String>(
										label: 'Fuel Type',
										value: _fuelType,
										options: const <String>['Petrol', 'Diesel', 'Hybrid', 'Electric', 'CNG'],
									...
									RentCarDropdownField<String>(
										label: 'Transmission',
										value: _transmission,
										options: const <String>['Manual', 'Automatic'],
										icon: Icons.settings_outlined,
										validator: (value) => value == null ? 'Select transmission' : null,
									...
									RentCarTextField(
										...
										keyboardType: TextInputType.number,
										validator: (value) => _numericValidator(value, 'seating capacity'),
										textInputAction: TextInputAction.next,
									),
									const SizedBox(height: 14),
									RentCarTextField(
									...
									RentCarTextField(
										...
										keyboardType: TextInputType.number,
										validator: (value) => _numericValidator(value, 'odometer reading'),
										textInputAction: TextInputAction.next,
									),
								],
							),
